...
def main():
    ...
    for i, p in enumerate(players, 1):
        print(f"   {i}. {p.name} - Position: {p.position}, Cash: ${p.cash:,}")
    ...
    print(f"💰 Final Cash: ${winner.cash:,}".center(70))
    
    # Final standings
    print(f"\n{'FINAL STANDINGS:'.center(70)}")
    print("-" * 70)
    sorted_players = sorted(players, key=lambda p: p.cash, reverse=True)
    for rank, p in enumerate(sorted_players, 1):
        medal = "🥇" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else "  "
        print(f"{medal} {p.name:20} - Cash: ${p.cash:>12,}")
